...
int main(int argc, char** argv) {
  ...
  // get start and goal nodes and num blocks
  begin_end begin_end_data = get_begin_end(is);
  Node begin = begin_end_data.begin;
  Node end = begin_end_data.end;
  int num_stacks = begin_end_data.num_stacks;
  int num_blocks = begin_end_data.num_blocks;

  string output_filename = argv[2];
  // open ofstream, if file does not exist, create it
  ofstream os(output_filename, fstream::out | fstream::app);

  // if first file, output column headers
  if (input_filename == "BWCHP/bwchp01.bwp") {
    os << "Filename, Status, Num. Goal Tests, Max Queue Size, Num. Stacks, Num. Blocks, Soln. Path Length\n";
  }
  ...
  a_star(begin, end, num_stacks, num_blocks, input_filename, screen_output, os);
